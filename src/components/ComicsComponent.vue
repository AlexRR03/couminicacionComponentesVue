<template>
  <div>
    ...
    <form v-on:submit.prevent="crearComic()">
        ...
        <button>CrearComic</button>
    </form>
    <div v-if="comicFav" class="verde">
        <h2 style="color:blue">{{comicFav.titulo}}</h2>
        <h2 style="color:yellow">{{comicFav.descripcion}}</h2>
    </div>
    <div id="comics" v-for="(comic,index) in comics" :key="comic">
            <ComicComponent :comic="comic" :index="index" v-on:seleccionarFavorito="seleccionarFavorito"
            v-on:eliminarComic="eliminarComic"></ComicComponent>
    </div>
  </div>
</template>
